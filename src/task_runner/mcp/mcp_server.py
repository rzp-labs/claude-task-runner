...
def configure_logging(level: str = "INFO") -> None:
    ...
    logger.add(
        sys.stderr, format="<level>{level: <8}</level> | {message}", level=level, colorize=True
    )
...
def main() -> int:
    ...
    # Info command
    subparsers.add_parser("info", help="Display server information")
    ...
    if args.command == "start":
        ...
        # Log startup info
        logger.info("Starting MCP server for Task Runner")
