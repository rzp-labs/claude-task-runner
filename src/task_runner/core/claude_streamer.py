...
def stream_claude_output(
    ...
    try:
        ...
        with open(task_file, "r") as input_file, open(result_file, "w") as result_output, open(
            error_file, "w"
        ) as error_output:
            ...
            while True:
                ...
                if process.poll() is not None and (
                    not process.stdout or not process.stdout.readable()
                ):
                ...
                if line:
                    ...
                    if not quiet and time.time() - last_output_time > 10:
                        logger.info(
                            f"Claude has been silent for {int(time.time() - last_output_time)}s"
                        )
                ...
                if not quiet and result_path.exists() and result_path.stat().st_size > 0:
                    with open(result_file, "r") as f:
                        ...
                        if "usage limit reached" in result_content.lower():
                            logger.error(
                                "CLAUDE USAGE LIMIT REACHED - Your account has reached its quota"
                            )
                            logger.error(
                                "Consider waiting until your quota resets or upgrading your Claude plan"
                            )
...
def clear_claude_context(claude_path: Optional[str] = None) -> bool:
    ...
    try:
        process = subprocess.run(
            cmd, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=10
        )
...
def run_claude_tasks(
    ...
    for i, task_file in enumerate(task_files):
        if not os.path.exists(task_file):
            ...
            results.append({"task_file": task_file, "success": False, "error": "File not found"})
            continue
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            if not os.path.exists(test_task_file):
                all_validation_failures.append(
                    "File creation test: Failed to create test task file"
                )
        except Exception as e:
        ...
        try:
            ...
            if missing_keys:
                all_validation_failures.append(
                    f"Parameter validation test: Missing keys in result: {missing_keys}"
                )
        ...
        try:
            ...
            if timeout_result.get("status") != "timeout":
                all_validation_failures.append(
                    f"Timeout test: Expected status 'timeout', got '{timeout_result.get('status')}'"
                )
            ...
            if result_file and os.path.exists(result_file):
                with open(result_file, "r") as f:
                    ...
                    if "TIMEOUT" not in content:
                        all_validation_failures.append(
                            "Timeout test: Expected timeout message in result file"
                        )
        except Exception as e:
        ...
        try:
            ...
            if result is not True and result is not False:
                all_validation_failures.append(
                    f"clear_claude_context test: Expected boolean result, got {type(result)}"
                )

            logger.info("Context clearing test completed")
        except Exception as e:
        ...
        try:
            ...
            missing_keys = [key for key in required_keys if key not in tasks_result]
            if missing_keys:
                all_validation_failures.append(
                    f"run_claude_tasks test: Missing keys in result: {missing_keys}"
                )

            # Check expected values
            if tasks_result.get("total_tasks") != 2:
                all_validation_failures.append(
                    f"run_claude_tasks test: Expected 2 total tasks, got {tasks_result.get('total_tasks')}"
                )

            if tasks_result.get("successful_tasks", 0) < 1:
                all_validation_failures.append(
                    "run_claude_tasks test: Expected at least 1 successful task"
                )
        except Exception as e:
