...
class TaskManager:
    ...
    def _find_claude_executable(self) -> str:
        ...
        try:
            ...
            if which_result.returncode == 0:
                ...
                if claude_path and os.access(claude_path, os.X_OK):
                    logger.info(
                        f"Found Claude at: {claude_path} in {time.time() - lookup_start:.4f}s"
                    )
                    return claude_path
        except Exception as e:
        ...
        logger.warning(
            f"Claude executable not found after {time.time() - lookup_start:.4f}s, using 'claude' as fallback"
        )
        return "claude"
    ...
    def run_task(
        ...
        try:
            ...
            # Note: fast_mode is deprecated, keeping for compatibility
            ...
            logger.info(
                f"{'Using streaming approach' if use_streaming else 'Using simple file redirection'} for task: {task_name}"
            )
            ...
            if use_streaming:
                ...
                # Note: --no-auth-check has been removed from Claude CLI
                # if fast_mode:
                #     cmd_parts.append("--no-auth-check")
                if skip_permissions:
            ...
            try:
                ...
                # Check result
                logger.info(
                    f"Command completed with exit code {exit_code} in {execution_time:.2f}s"
                )
                ...
                # Task timed out - will update state below
                exit_code = -1
