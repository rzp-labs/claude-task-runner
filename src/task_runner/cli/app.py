...
app = typer.Typer(help="Claude Task Runner", rich_markup_mode="rich")
...
def run(
    ...
    no_table_repeat: bool = typer.Option(
        False,
        "--no-table-repeat",
        help="Display the table only once, don't repeat after each task (better with streaming)",
    ),
    dangerous_mode: bool = typer.Option(
        False,
        "--dangerous",
    ...
    if claude_path:
        manager.claude_path = claude_path
    ...
    try:
        ...
        if json_output:
            ...
            results = manager.run_all_tasks(
                ...
                fast_mode=False,  # fast_mode is deprecated
                demo_mode=quick_demo,
                use_streaming=not no_streaming,
                skip_permissions=dangerous_mode,
            )
            print_json(results)
        else:
            ...
            for task_file in task_files:
                ...
                if no_table_repeat:
                    ...
                    total = summary["total"]
                    print_info(
                        f"Progress: {completed}/{total} tasks completed ({int(completed/total*100)}%)"
                    )
                else:
...
def status(
    ...
    if json_output:
        print_json({"tasks": task_state, "summary": summary})
    else:
        # Display dashboard components
        components = create_dashboard(
            task_state, manager.current_task, manager.current_task_start_time
        )
...
def create(
    ...
    task_list: Optional[Path] = typer.Argument(
        None, help="Path to task list file", callback=validate_task_list_file
    ),
    base_dir: Path = typer.Option(
        Path.home() / "claude_task_runner",
        help="Base directory for tasks and results",
